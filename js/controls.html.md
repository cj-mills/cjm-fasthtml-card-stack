...
# --- Dead-zone fix: decreaseWidth snaps to effective width first ---
# Helper that reads inner.offsetWidth and converts to rem for comparison.
assert "ns._getEffectiveWidthRem" in js
assert "offsetWidth" in js
assert "fontSize" in js  # Root font size used for px→rem conversion
# decreaseWidth uses Math.min(current, effective) as the decrement base.
assert "Math.min(current, effective)" in js

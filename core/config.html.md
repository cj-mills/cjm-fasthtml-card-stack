...
assert _DEFAULT_FOCUS_Z_INDEX == "z-1"
...
def CardStackStyleConfig(
    ...
    viewport_padding_y:str='0.5rem',
    focus_ring:str='ring-1 ring-[color-mix(in_oklch,var(--color-primary),transparent_50%)]',
    focus_shadow:str='shadow-lg shadow-primary', focus_border_radius:str='rounded-box', focus_z_index:str='z-1'
)->None:
...
assert style.focus_z_index == "z-1"
assert not hasattr(style, 'focus_padding_x')  # Removed — shadow on section, not inside it
...
# focus_padding_* removed — focus styling on section, not inside it
assert "focus-padding" not in css
...
reused across requests. The `prefix` drives `CardStackHtmlIds` and
`CardStackButtonIds` generation for multi-instance support.
...
def CardStackConfig(
    ...
    card_width_step:int=1, card_scale_min:int=50, card_scale_max:int=200, card_scale_step:int=10,
...
assert config.card_width_step == 1

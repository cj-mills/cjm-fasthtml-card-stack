...
# %% auto #0
__all__ = ['render_slot_card', 'render_all_slots_oob', 'render_card_stack_scrollbar', 'render_viewport']
...
from cjm_fasthtml_tailwind.utilities.flexbox_and_grid import (
    flex_display, flex_direction, justify, items, gap, grid_display, grow
)
...
# %% ../../nbs/components/viewport.ipynb #o5pd0z8v3hs
def _map_to_scrollbar(
    ...
    sb_state = ScrollbarState(
        position=state.focused_index,
        visible_count=state.visible_count,
        total_items=total_items,
        max_position=max(0, total_items - 1),  # Focused-index model: range is [0, total-1]
        thumb_ratio=1 / total_items if total_items > 0 else None,  # Single focused item
    )
    sb_config = ScrollbarConfig(
        prefix=config.prefix,
        show_scrollbar=config.show_scrollbar,
        auto_hide=False,  # Card stack scrollbar always visible (single-focus model)
    )
    sb_ids = ScrollbarIds(prefix=config.prefix)
...
def render_card_stack_scrollbar(
    ...
    sb_state, sb_config, sb_ids = _map_to_scrollbar(state, config, total_items)
    sb = render_scrollbar(sb_state, sb_config, sb_ids)
    if oob:
...
def render_viewport(
    ...
    # Scrollbar alongside card stack in a flex row.
    # Always include when show_scrollbar=True — the scrollbar lib handles
    # auto-hide via the hidden class based on its auto_hide config.

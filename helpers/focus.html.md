...
def render_focus_oob(
    ...
    total_items:Optional=None, # Total item count (emitted as data-total-items for client-side boundary checks)
...
# Test total_items emitted as data-total-items for boundary guard
result = render_focus_oob(5, ids, total_items=42)
html = to_xml(result[0])
assert 'data-total-items="42"' in html
assert 'value="5"' in html

# Test total_items omitted when None (backward compat)
result = render_focus_oob(5, ids)
html = to_xml(result[0])
assert 'data-total-items' not in html
print("Total items boundary-guard tests passed!")
```

    Custom form input name test passed!
    Total items boundary-guard tests passed!

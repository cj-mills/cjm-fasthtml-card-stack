...
def create_card_stack_focus_zone(
    ...
    label:Optional=None, # Human-readable label for keyboard-hints display (falls back to zone id)
...
``` python
# Test label pass-through to FocusZone (cjm-fasthtml-keyboard-navigation 0.0.22+).
# The label drives per-zone section headers in the keyboard-hints modal
# (e.g., "Text Segmentation — Navigation" instead of raw zone id).

# Default: label is None → FocusZone.label is None → display falls back to zone id
zone_no_label = create_card_stack_focus_zone(CardStackHtmlIds(prefix="cs-x"))
assert zone_no_label.label is None
assert zone_no_label.get_display_label() == zone_no_label.id  # falls back to id

# Custom label: passes through to FocusZone.label
zone_with_label = create_card_stack_focus_zone(
    CardStackHtmlIds(prefix="cs-y"),
...
# Label composes with other params (regression guard: no parameter ordering bug)
zone_combined = create_card_stack_focus_zone(
    CardStackHtmlIds(prefix="cs-z"),
    on_focus_change="onMyFocus",
    label="VAD Alignment",
    data_attributes=("segment-index",),
)
assert zone_combined.label == "VAD Alignment"
assert zone_combined.on_focus_change == "onMyFocus"
assert zone_combined.data_attributes == ("segment-index",)
print("Label pass-through tests passed!")
```

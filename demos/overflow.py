"""Overflow demo — tests focused card scrolling when content exceeds viewport height."""
...
from demos.data import OVERFLOW_ITEMS
...
def setup(route_prefix="/overflow"):
    """Set up the overflow demo.

    Returns dict with config, ids, btn_ids, router, urls, state management,
    and page rendering dependencies.
    """
    config = CardStackConfig(prefix="overflow", click_to_focus=True)
    ...
    return dict(
        ...
        get_items=get_items,
        render_card=render_card,
        container_id="overflow-demo-container",
        title="Overflow Demo",
        description="Tests focused card scrolling when content exceeds viewport. Items #3 and #8 are intentionally oversized.",
        progress_label="Item",
